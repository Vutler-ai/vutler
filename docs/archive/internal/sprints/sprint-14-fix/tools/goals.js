/**
 * tools/goals.js [PATCHED]
 * CRUD operations sur la table goals
 * 
 * FIXES:
 * - target_date → deadline (real column name)
 * - All queries use tenant_vutler. prefix
 */
...
const TOOL_DEFINITIONS = [
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        deadline: { type: 'string', description: 'Target completion date (ISO 8601, optional)' },
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        deadline: { type: 'string', description: 'New target date (optional)' }
      },
      required: ['goal_id']
    }
  },
...
class GoalsToolHandler {
    ...
    try {
      ...
      const result = await this.pool.query(
        `INSERT INTO tenant_vutler.goals
         (workspace_id, agent_id, title, description, deadline, priority, status, progress, created_at, updated_at)
         VALUES ('00000000-0000-0000-0000-000000000001', $1, $2, $3, $4, $5, $6, 0, NOW(), NOW())
         RETURNING id, workspace_id, agent_id, title, description, deadline, priority, status, progress, created_at`,
        [agent_id, title, description, deadline, priority, status]
      );
    ...
    try {
      ...
      const result = await this.pool.query(
        `UPDATE tenant_vutler.goals
         SET ${fields.join(', ')}
         WHERE id = $${paramIndex}
         RETURNING id, workspace_id, agent_id, title, description, deadline, priority, status, progress, updated_at`,
        values
      );
  ...
  async listGoals(args) {
    try {
      ...
      const result = await this.pool.query(
        `SELECT id, workspace_id, agent_id, title, description, deadline, priority, status, progress, created_at, updated_at
  ...
  async getGoal(args) {
    try {
      ...
      const result = await this.pool.query(
        `SELECT id, workspace_id, agent_id, title, description, deadline, priority, status, progress, phases, checkins, created_at, updated_at
         FROM tenant_vutler.goals
         WHERE id = $1`,
        [goal_id]
      );
