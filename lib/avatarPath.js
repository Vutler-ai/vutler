...
const SVG_AVATAR_SLUGS = new Set([
  'operations-oracle',
  'sales-pathfinder',
  'growth-catalyst',
  'finance-warden',
...
function normalizeStoredAvatar(avatar, options = {}) {
  ...
  if (value.startsWith('/static/avatars/')) {
    ...
    if (value.toLowerCase().endsWith('.svg') && SVG_AVATAR_SLUGS.has(slug)) {
      return `/static/avatars/${slug}.svg`;
    }
  ...
  if (SIMPLE_TOKEN_RE.test(value)) {
    ...
    if (SVG_AVATAR_SLUGS.has(slug)) return `/static/avatars/${slug}.svg`;
    if (KNOWN_AVATAR_SLUGS.has(slug)) return `/static/avatars/${slug}.png`;
    return buildSpriteAvatar(slug);
  }
