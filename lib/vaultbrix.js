...
// Fallback: require VAULTBRIX_PASSWORD + known host — DATABASE_URL is preferred
if (!pgConfig) {
  const pw = process.env.VAULTBRIX_PASSWORD;
  const host = process.env.VAULTBRIX_HOST || 'REDACTED_DB_HOST';
  const port = parseInt(process.env.VAULTBRIX_PORT || '5433', 10);
  if (!pw) {
    console.error('[Vaultbrix] FATAL: DATABASE_URL not set and VAULTBRIX_PASSWORD missing — DB connection will fail!');
  }
  pgConfig = {
    host,
    port,
    database: 'postgres',
    user: process.env.VAULTBRIX_USER || 'tenant_vutler',
    password: pw,
    ssl: false,
    max: 10,
    idleTimeoutMillis: 30000,
  ...
  console.log(`[Vaultbrix] Config from env fallback: ${pgConfig.user}@${pgConfig.host}:${pgConfig.port}/${pgConfig.database}`);
}
