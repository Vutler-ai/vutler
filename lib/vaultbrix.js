...
if (!pgConfig) {
  ...
  if (!pgConfig.password) {
    console.error("[Vaultbrix] CRITICAL: VAULTBRIX_PASSWORD not set and no DATABASE_URL — PG will fail!");
  } else {
    console.log(`[Vaultbrix] Config from hardcoded fallback: ${pgConfig.user}@${pgConfig.host}:${pgConfig.port}/${pgConfig.database}`);
  }
}
