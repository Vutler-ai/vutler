...
import type { NexusNode, NexusStats, Agent } from '@/lib/api/types';
...
export default function NexusPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to load Nexus data. Please refresh.');
      setNodes([]);
    } finally {
      setLoading(false);
    }
    ...
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      {error && (
        <div className="bg-red-500/10 border border-red-500/30 text-red-400 px-4 py-3 rounded-lg text-sm flex items-center gap-2">
          <span>{error}</span>
          <button onClick={fetchData} className="ml-auto text-red-300 hover:text-white underline text-xs">Retry</button>
        </div>
      )}
