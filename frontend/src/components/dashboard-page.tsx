...
// Inline button helpers (previously from topbar.tsx)
function TopbarButton({
  ...
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`flex items-center space-x-2 px-4 py-2 rounded-lg font-medium text-sm motion-safe:transition-colors motion-safe:duration-200 cursor-pointer focus:outline-none focus:ring-2 focus:ring-[#3b82f6] focus:ring-offset-2 focus:ring-offset-[#08090f] ${variants[variant]}`}
    >
...
export default function DashboardPage() {
  ...
  const stats = data && data.stats ? [
    {
      label: 'Total Agents',
      value: (data.stats.totalAgents || 0).toString(),
      icon: StatIcons.Agents,
      color: 'blue' as const,
    },
    {
      label: 'Active Agents',
      value: (data.stats.activeAgents || 0).toString(),
      icon: StatIcons.Agents,
      color: 'green' as const,
    },
    {
      label: 'Messages Today',
      value: (data.stats.messagesToday || 0).toLocaleString(),
    ...
    <>
      ...
      <section className="bg-[#14151f] border border-[rgba(255,255,255,0.07)] rounded-xl p-6 mb-8" aria-labelledby="quick-actions-title">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <button onClick={() => window.location.href = "/usage"} className="flex items-center space-x-3 p-4 rounded-lg bg-[#0e0f1a] hover:bg-[#1a1b2e] border border-[rgba(255,255,255,0.07)] hover:border-[rgba(255,255,255,0.15)] motion-safe:transition-all motion-safe:duration-200 group cursor-pointer focus:outline-none focus:ring-2 focus:ring-[#3b82f6] focus:ring-offset-2 focus:ring-offset-[#14151f]">
          ...
          <button onClick={() => window.location.href = "/templates"} className="flex items-center space-x-3 p-4 rounded-lg bg-[#0e0f1a] hover:bg-[#1a1b2e] border border-[rgba(255,255,255,0.07)] hover:border-[rgba(255,255,255,0.15)] motion-safe:transition-all motion-safe:duration-200 group cursor-pointer focus:outline-none focus:ring-2 focus:ring-[#3b82f6] focus:ring-offset-2 focus:ring-offset-[#14151f]">
      ...
      {/* Getting Started Checklist - shown when no agents */}
      {!loading && (!data?.agents || data.agents.length === 0) && (
        <section className="bg-gradient-to-br from-[#1e293b] to-[#14151f] border border-[#3b82f6]/30 rounded-xl p-8 mb-8 text-center">
          <div className="text-5xl mb-4">🚀</div>
          <h2 className="text-2xl font-bold text-white mb-2">Create Your First Agent</h2>
          <p className="text-[#94a3b8] mb-6 max-w-md mx-auto">Get started by creating an AI agent or use our onboarding wizard for a guided setup.</p>
          <div className="flex justify-center gap-4">
            <a href="/onboarding" className="px-6 py-3 bg-[#3b82f6] hover:bg-[#2563eb] text-white rounded-xl font-medium transition-colors">Start Onboarding →</a>
            <a href="/agents/new" className="px-6 py-3 bg-[#1e293b] hover:bg-[#334155] text-white border border-[rgba(255,255,255,0.1)] rounded-xl font-medium transition-colors">Create Manually</a>
          </div>
        </section>
      )}
      ...
      {!loading && data?.agents && data.agents.length > 0 && (
        <section className="bg-[#14151f] border border-[rgba(255,255,255,0.07)] rounded-xl p-6 mb-8">
