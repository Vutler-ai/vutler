...
export default function AgentsTable({ agents, onAgentClick }: AgentsTableProps) {
  ...
  const statusConfig: Record<string, {color:string,text:string,label:string}> = { online: { color: 'bg-[#22c55e]', text: 'text-[#22c55e]', label: 'Online' }, busy: { color: 'bg-[#ef4444]', text: 'text-[#ef4444]', label: 'Busy' },
              ...
              >
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="inline-flex items-center space-x-2 px-2.5 py-1 rounded-full bg-[rgba(255,255,255,0.05)] whitespace-nowrap">
                    <span className={`w-2 h-2 rounded-full ${( statusConfig[agent.status] || statusConfig.offline ).color}`} aria-hidden="true" />
                    <span className={`text-sm font-medium ${( statusConfig[agent.status] || statusConfig.offline ).text}`}>
                      {( statusConfig[agent.status] || statusConfig.offline ).label}
                    </span>
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between mb-2">
                  ...
                  <div className="inline-flex items-center space-x-1 ml-2 px-2 py-0.5 rounded-full bg-[rgba(255,255,255,0.05)] whitespace-nowrap">
                    <span className={`w-2 h-2 rounded-full ${( statusConfig[agent.status] || statusConfig.offline ).color}`} aria-hidden="true" />
                    <span className={`text-xs font-medium ${( statusConfig[agent.status] || statusConfig.offline ).text}`}>
                      {( statusConfig[agent.status] || statusConfig.offline ).label}
                    </span>
                  </div>
                </div>
