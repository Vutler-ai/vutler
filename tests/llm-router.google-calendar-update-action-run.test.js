...
describe('llmRouter google calendar update action runs', () => {
  let recordedBodies;
  let chat;
  let actionRuns;
  let db;
              ...
              tool_calls: [
                {
                  ...
                  type: 'function',
                  function: {
                    name: 'skill_google_calendar_update',
        ...
        choices: [
          {
            ...
            message: {
              role: 'assistant',
              content: 'Kickoff event updated in Google Calendar.',
            },
          },
        ],
  ...
  test('records chat_action_runs for google calendar update skills', async () => {
    const result = await chat(
      {
        ...
        provider: 'openai',
        model: 'gpt-5.4',
        skills: ['google_calendar_update'],
        system_prompt: 'You update meetings.',
      },
      [{ role: 'user', content: 'Move the kickoff meeting by one hour.' }],
      db,
      {
        chatActionContext: {
          workspaceId: 'ws-1',
    ...
    expect(actionRuns[0]).toMatchObject({
      ...
      channel_id: 'chan-1',
      action_key: 'google_calendar_update',
      adapter: 'skill',
      status: 'success',
    });
          ...
          actions: [
            expect.objectContaining({
              ...
              params: expect.objectContaining({
                skill_key: 'google_calendar_update',
              }),
            }),
          ],
        }),
