/**
 * tools/tasks.js [PATCHED]
 * CRUD operations sur la table tasks
 * 
 * FIXES:
 * - assigned_to → assignee (real column name)
...
const TOOL_DEFINITIONS = [
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        assignee: { type: 'string', description: 'Agent ID to assign to (optional)' },
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        assignee: { type: 'string', description: 'New assignee agent ID (optional)' },
  ...
  },
  {
    ...
    input_schema: {
      type: 'object',
      properties: {
        assignee: { type: 'string', description: 'Filter by agent ID (optional)' },
...
class TasksToolHandler {
    ...
    try {
      ...
      const result = await this.pool.query(
        `INSERT INTO tenant_vutler.tasks
         (title, description, assignee, priority, due_date, status, created_at, updated_at, workspace_id)
         VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW(), '00000000-0000-0000-0000-000000000001')
         RETURNING id, title, description, assignee, priority, due_date, status, created_at`,
    ...
    try {
      ...
      const result = await this.pool.query(
        `UPDATE tenant_vutler.tasks
         SET ${fields.join(', ')}
         WHERE id = $${paramIndex}
         RETURNING id, title, description, assignee, priority, due_date, status, updated_at`,
        values
      );
  ...
  async listTasks(args) {
    try {
      const { assignee, status, priority, limit = 20 } = args;
      ...
      const result = await this.pool.query(
        `SELECT id, title, description, assignee, priority, due_date, status, created_at, updated_at
